...
| `MetaBoClipHub` | AImd-facing MetaboClip result hub for role assets, score tables, reports, and final ranking handoff | MetaBoClipBridge outputs | `data/data_output/metaboclip/` |
...
`data_input` is for user-provided starting files only. `data_output` is for files generated by AImd modules. If a local package still has older `data/protein_input`, `data/ligand_input`, or `data/input` folders, run:

```bash
python scripts/migrate_data_layout.py --root .
```

The migration helper copies local inputs into the canonical layout and writes project-root-relative input manifests. It does not delete the original local folders.
