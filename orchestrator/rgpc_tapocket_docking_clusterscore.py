...
def run_workflow(config_path: str | Path) -> None:
    ...
    print("[AImdWorkflow] RGPC → TApocket → Docking → ClusterScore workflow finished.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run AImd RGPC + TApocket + Docking + ClusterScore workflow")
    parser.add_argument("--config", required=True, help="Path to configs/workflows/rgpc_tapocket_docking_clusterscore.yaml")
